...
class _String_base
{
  ...
  { return  _M_using_static_buf() ? _DEFAULT_SIZE - (_M_finish - _M_buffers._M_static_buf) : _M_buffers._M_end_of_storage - _M_finish; }
    ...
    _Tp* _M_Start()
      { return _M_start_of_storage._M_data; }
    _Tp const* _M_End()
      const { return _M_end_of_storage; }
    _Tp* _M_End()
    ...
    _String_base(__move_source<_Self> src) :
        ...
        _M_end_of_storage(0),
        _M_finish(0),
        _M_start_of_storage( allocator_type(), (_Tp*)0)
#endif
      { _M_swap(src.get()); }
#endif
    ...
    void _M_swap( _Self& __s )
      {
        ...
        // copy( _M_buffers._M_static_buf, _M_buffers._M_static_buf + _DEFAULT_SIZE, s._M_buffers._M_static_buf );
        // copy( tmp, tmp + _DEFAULT_SIZE, _M_buffers._M_static_buf );
        // _STLP_STD::swap( _M_finish, s._M_finish );
