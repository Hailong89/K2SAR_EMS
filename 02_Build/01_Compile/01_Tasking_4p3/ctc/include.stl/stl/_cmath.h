...
#if __CPTC__ // In the Tricore tool-chain fabs and fabsf are macros mapping onto built-ins.
namespace std {
using ::__fabs;
using ::__fabsf;
}
...
#  if !defined(_STLP_HAS_NO_NAMESPACES) && !defined(__SUNPRO_CC) && !defined(__TASKING__)
...
#if defined (__BORLANDC__) || defined (__TASKING__)
 ...
 */
    ...
    !defined (__WATCOMC__)
#  if defined(__TASKING__)
//inline double abs(double __x) { return _STLP_CMATH_FUNC_NAMESPACE::fabs(__x); }
#  else /* !defined(__TASKING__) */
...
_STLP_DEF_MATH_INLINE((fabs), fabs) // wrapped fabs in parentheses to avoid it being substituted by the TriCore's fabs macro
