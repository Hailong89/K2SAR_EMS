...
inline void _Copy_Construct_aux( _T1* __p, const _T2& __val, const false_type& /* is_fundamental */ )
{
  new(__p) _T1(__val);
}

template <class _T1, class _T2>
inline void _Copy_Construct_aux( _T1* __p, const _T2& __val, const true_type& /* is_fundamental */ )
{
  // We use binary copying for POD types since it results
  // in a considerably better code at least on MSVC.
  *__p = __val;
...
inline void _Copy_Construct( _T1* __p, const _T2& __val )
{
  ...
  memset((char*)__p, _STLP_SHRED_BYTE, sizeof(_T1));
#endif
  _Copy_Construct_aux( __p, __val, integral_constant<bool,is_fundamental<_T1>::value && is_fundamental<_T2>::value>() );
}
...
struct __is_stateless_alloc :
    public integral_constant<bool,is_trivial<_T>::value || is_empty<_T>::value>
{ };
