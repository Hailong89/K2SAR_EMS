...
* Developed using:
* Function: Determine the frequency the CPU is running at (TC1798)
*
...
#ifndef DEF_FRQ
#define DEF_FRQ			20000000U	/* TC1798 quartz frequency is 20 MHz */
#endif /* DEF_FRQ */
...
void set_cpu_frequency(void)
{
	...
	if (!SCU_PLLSTAT.bits.PWDSTAT)
	{
		...
		pllcon0.reg = 0x00010000;	/* not documented bit PLLPWD */
		pllcon1.reg = 0;
...
#ifdef ENABLE_ICACHE
		/* enable instruction cache (PCON0) */
		unlock_wdtcon();
		PCON0.bits.PCBYP = 0;
		lock_wdtcon();
#endif /* ENABLE_ICACHE */
